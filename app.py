...
def index(m3u8):
    ...
    source = m3u8
    source = source.replace('https://volestream-1e010.kxcdn.com/', '')
    ...
    tsal = ts.text
    tsal = tsal.replace(videoid+'_','https://volestream-1e010.kxcdn.com/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/'+videoid+'/1/'+videoid+'_')
    if "internal" in tsal:
        tsal = tsal.replace('internal','https://volestream-1e010.kxcdn.com/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/'+videoid+'/1/internal')
    if "segment" in tsal:
        tsal = tsal.replace('\n'+'media','\n'+'https://volestream-1e010.kxcdn.com/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/'+videoid+'/1/media')
    return tsal
...
def getstream():
    ...
    if param == "getts":
        source = request.url
        source = source.replace('https://volestream-1e010.kxcdn.com/getstream?param=getts&source=','')
        source = source.replace('%2F','/')
        source = source.replace('%3F','?')
        headers = {
            'accept': '*/*',
