...
def index(m3u8):
    ...
    source = m3u8
    source = source.replace('https://deneme.maltinok.com/', '')
...
def getm3u8():
    source = request.url
    source = source.replace('https://deneme.maltinok.com/getm3u8?source=', '')
    source = source.replace('%2F', '/')
    source = source.replace('%3F', '?')
    headers = {
        "accept": "*/*",
    ...
    ts = requests.get(source, headers=headers)
    tsal = ts.text
    tsal = tsal.replace(videoid+'_','https://deneme.maltinok.com/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/'+videoid+'/1/'+videoid+'_')
    return tsal
...
def getstream():
    ...
    if param == "getm3u8":
        ...
        if "FullscreenAllowed" in r.text:
            ...
            veri = veri.replace('edge100','edge10')
            if "m3u8" in veri:
                '''return "https://deneme.maltinok.com/getm3u8?source="+veri+'&videoid='+videoid'''
                return "https://deneme.maltinok.com/"+veri+'&videoid='+videoid
        else:
            return "Veri yok"
