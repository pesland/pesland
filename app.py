...
def index(m3u8):
    ...
    source = m3u8.replace('https://neset-baba-sigara.keremihsanozer.workers.dev/', '')
    ...
    tsal = ts.text.replace(videoid+'_', f'https://neset-baba-sigara.keremihsanozer.workers.dev/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/{videoid}/1/{videoid}_')
    if 'internal' in tsal:
        tsal = tsal.replace('internal', f'https://neset-baba-sigara.keremihsanozer.workers.dev/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/{videoid}/1/internal')
    if 'segment' in tsal:
...
def getstream():
    ...
    if param == "getts":
        source = request.url
        source = source.replace('https://neset-baba-sigara.keremihsanozer.workers.dev/getstream?param=getts&source=','')
        source = source.replace('%2F','/')
        source = source.replace('%3F','?')
        headers = {
            'accept': '*/*',
