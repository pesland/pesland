...
def index(m3u8):
    ...
    tsal = ts.replace(videoid + "_", f"https://cdn.esraerol.workers.dev/https://plankton-app-ljeqg.ondigitalocean.app/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/{videoid}/1/{videoid}_")
    if "internal" in tsal:
    ...
    if "segment" in tsal:
        tsal = tsal.replace("\nmedia", f"\nhttps://cdn.esraerol.workers.dev/https://plankton-app-ljeqg.ondigitalocean.app/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/{videoid}/1/media")

    return Response(tsal, status=response.status, headers=translateResponseLocation(response.headers))
