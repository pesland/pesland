...
def index(m3u8):
    ...
    source = m3u8.replace('https://shark-app-oy2nm.ondigitalocean.app/', '')
    ...
    tsal = tsal.replace(videoid + '_', 'https://googleuserproxy.global.ssl.fastly.net/getstream?param=getts&source=https://googleusercontent1x.global.ssl.fastly.net/hls-live/' + videoid + '/1/' + videoid + '_')

    if "internal" in tsal:
        tsal = tsal.replace('internal', 'https://googleuserproxy.global.ssl.fastly.net/getstream?param=getts&source=https://googleusercontent1x.global.ssl.fastly.net/hls-live/' + videoid + '/1/internal')
...
def getm3u8():
    source = request.url.replace('https://shark-app-oy2nm.ondigitalocean.app/getm3u8?source=', '')
    source = source.replace('%2F', '/')
    source = source.replace('%3F', '?')
...
def getstream():
    ...
    if param == "getts":
        source = request.url.replace('https://shark-app-oy2nm.ondigitalocean.app/getstream?param=getts&source=', '')
        source = source.replace('%2F', '/')
        source = source.replace('%3F', '?')
    ...
    if param == "getm3u8":
        ...
        if "FullscreenAllowed" in r.text:
            ...
            if "m3u8" in veri:
                return "https://shark-app-oy2nm.ondigitalocean.app/" + veri + '&videoid=' + videoid
