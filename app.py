...
@cache.cached(timeout=60)  # 60 saniye önbellekte sakla
def get_data_from_source(url, headers):
    ts = requests.get(url, headers=headers)
    return ts.text

def make_async_requests(urls, headers):
    rs = (grequests.get(u, headers=headers) for u in urls)
    responses = grequests.map(rs)
    return responses[0]

def make_parallel_request(url, headers):
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(make_request, url, headers)]
...
def index(m3u8):
    ...
    tsal = get_data_from_source(source, headers)
    tsal = tsal.replace(videoid + '_', 'https://worker-holy-shadow-d82a.volestream--geldi-kizlarr.workers.dev/https://seashell-app-cdk6o.ondigitalocean.app/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/' + videoid + '/1/' + videoid + '_')

    if "internal" in tsal:
        tsal = tsal.replace('internal',
                            'https://worker-holy-shadow-d82a.volestream--geldi-kizlarr.workers.dev/https://seashell-app-cdk6o.ondigitalocean.app/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/' + videoid + '/1/internal')
...
def getm3u8():
    ...
    ts = make_parallel_request(source, headers)
    tsal = ts.text
    tsal = tsal.replace(videoid + '_',
                        'https://seashell-app-cdk6o.ondigitalocean.app/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/' + videoid + '/1/' + videoid + '_')
