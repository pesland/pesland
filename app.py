...
def index(m3u8):
    ...
    source = m3u8
    source = source.replace('https://neset-baba-sigara.keremihsanozer.workers.dev/', '')
...
def getm3u8():
    source = request.url
    source = source.replace('https://neset-baba-sigara.keremihsanozer.workers.dev/getm3u8?source=', '')
    source = source.replace('%2F', '/')
    source = source.replace('%3F', '?')
    headers = {
        "accept": "*/*",
