...
def index(m3u8):
    ...
    tsal = ts.text.replace(videoid+'_', f'https://cors-proxy.fringe.zone/https://neset-baba-sigara.keremihsanozer.workers.dev/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/{videoid}/1/{videoid}_')
    if 'internal' in tsal:
        tsal = tsal.replace('internal', f'https://cors-proxy.fringe.zone/https://neset-baba-sigara.keremihsanozer.workers.dev/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/{videoid}/1/internal')
    if 'segment' in tsal:
        tsal = tsal.replace('\nmedia', f'\nhttps://cors-proxy.fringe.zone/https://neset-baba-sigara.keremihsanozer.workers.dev/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/{videoid}/1/media')
    return tsal
...
def getstream():
    ...
    if param == "getm3u8":
        ...
        if "FullscreenAllowed" in r.text:
            ...
            veri = veri.replace('edge100','edge10')
            veri = re.sub(r'https:\/{2,}','https://', veri)
            veri = re.sub(r'edge\d+\.xmediaget\.com','edge10.xmediaget.com',veri)
            veri = re.sub(r'edge\d+','edge10',veri)
            veri = veri.replace('edge10.xmediaget.com/hls-live','erdoganladevam.herokuapp.com')
            return veri
