...
def index(m3u8):
    ...
    source = m3u8
    source = source.replace('https://memetsekmn-267a5e0b624a.herokuapp.com/', '')
...
def getm3u8():
    source = request.url
    source = source.replace('https://memetsekmn-267a5e0b624a.herokuapp.com/getm3u8?source=', '')
    source = source.replace('%2F', '/')
    source = source.replace('%3F', '?')
    headers = {
        "accept": "*/*",
    ...
    ts = requests.get(source, headers=headers)
    tsal = ts.text
    tsal = tsal.replace(videoid+'_','https://memetsekmn-267a5e0b624a.herokuapp.com/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/'+videoid+'/1/'+videoid+'_')
    return tsal
...
def getstream():
    ...
    if param == "getts":
        source = request.url
        source = source.replace('https://memetsekmn-267a5e0b624a.herokuapp.com/getstream?param=getts&source=','')
        source = source.replace('%2F','/')
        source = source.replace('%3F','?')
        headers = {
            'accept': '*/*',
    ...
    if param == "getm3u8":
        ...
        if "FullscreenAllowed" in r.text:
            ...
            veri = veri.replace('edge100','edge10')
            if "m3u8" in veri:
                '''return "https://memetsekmn-267a5e0b624a.herokuapp.com/getm3u8?source="+veri+'&videoid='+videoid'''
                return "https://memetsekmn-267a5e0b624a.herokuapp.com/"+veri+'&videoid='+videoid
        else:
            return "Veri yok"
