...
def index(m3u8):
    ...
    tsal = ts.text
    tsal = tsal.replace(videoid+'_','https://izstream.herokuapp.com/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/'+videoid+'/1/'+videoid+'_')
    if "internal" in tsal:
        tsal = tsal.replace('internal','https://izstream.herokuapp.com/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/'+videoid+'/1/internal')
    if "segment" in tsal:
        tsal = tsal.replace('\n'+'media','\n'+'https://izstream.herokuapp.com/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/'+videoid+'/1/media')
    return tsal
...
def getstream():
    ...
    if param == "getts":
        source = request.url
        source = source.replace('https://izstream.herokuapp.com/getstream?param=getts&source=','')
        source = source.replace('%2F','/')
        source = source.replace('%3F','?')
        headers = {
            'accept': '*/*',
    ...
    if param == "getm3u8":
        ...
        if "FullscreenAllowed" in r.text:
            ...
            veri = veri.replace('edge100','edge10')
            if "m3u8" in veri:
                '''return "https://izstream.herokuapp.com/getm3u8?source="+veri+'&videoid='+videoid'''
                return "https://izstream.herokuapp.com/"+veri+'&videoid='+videoid
        else:
            return "Veri yok"
