...
def getm3u8():
    ...
    ts = requests.get(source, headers=headers)
    tsal = ts.text
    tsal = tsal.replace(videoid+'_','https://onebet.volestream01.workers.dev/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/'+videoid+'/1/'+videoid+'_')
    return tsal
...
def getstream():
    ...
    if param == "getm3u8":
        ...
        if "FullscreenAllowed" in r.text:
            ...
            veri = veri.replace('edge100','edge10')
            if "m3u8" in veri:
                '''return "https://onebet.volestream01.workers.dev/getm3u8?source="+veri+'&videoid='+videoid'''
                return "https://onebet.volestream01.workers.dev/"+veri+'&videoid='+videoid
        else:
            return "Veri yok"
