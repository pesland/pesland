...
def index(m3u8):
    ...
    ts = requests.get('https://late-breeze-c588.kkjfjfjsfjsfjsjfj.workers.dev' + source, headers=headers)
...
def getm3u8():
    ...
    tsal = ts.text
    videoid = request.args.get('videoid')
    tsal = tsal.replace(videoid + '_', f'https://late-breeze-c588.kkjfjfjsfjsfjsjfj.workers.dev/getstream?param=getts&source=https://edge10.xmediaget.com/hls-live/{videoid}/1/{videoid}_')
    return tsal
...
def getstream():
    ...
    if param == "getts":
        source = request.url.replace('https://late-breeze-c588.kkjfjfjsfjsfjsjfj.workers.dev/getstream?param=getts&source=', '')
        source = source.replace('%2F', '/')
        source = source.replace('%3F', '?')
        headers = {
            'accept': '*/*',
